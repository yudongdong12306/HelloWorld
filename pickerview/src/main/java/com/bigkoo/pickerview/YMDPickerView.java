...
import com.gsh.utils.android.LocaleUtils;
...
public class YMDPickerView<T> extends BasePickerView implements View.OnClickListener {
    ...
    public enum Type {
        YEAR_MONTH_DAY, MONTH_YEAR, YEAR_MONTH, MONTH_DAY
    }
     ...
     * @param title       标题
     * @param initTime    初始时间 秒
     * @param englishType 英文状态下需要显示的style样式,true为YEAR_MONTH_DAY,false为MONTH_YEAR
     */
    public YMDPickerView(Context context, Type type, String title, long initTime, boolean englishType) {
        super(context);
        this.mContext = context;
        this.mType = type;
        this.mTitle = title;
    ...
    private void init1() {
        ...
        //中国需要有单位
        if (LocaleUtils.isChinese(mContext)) {
    ...
    private void init2(boolean englishType) {
        LayoutInflater.from(mContext).inflate(R.layout.pickerview_ymd, contentContainer);
    ...
    private void initData() {
        ...
        if (mType == Type.YEAR_MONTH_DAY || mType == Type.YEAR_MONTH) {
            ...
            mMonthWv.setAdapter(new ArrayWheelAdapter(PickerViewUtils.getMonths()));
            mMonthWv.setCurrentItem(month);
            setDayAdapter(Calendar.getInstance().get(Calendar.YEAR), month);
            if (day > mDayWv.getItemsCount()) {
                //不可能出现
                day = mDayWv.getItemsCount();
            }
    ...
    public void hideCancelButton(boolean isHide) {
        mCancelBtn.setVisibility(isHide ? View.INVISIBLE : View.VISIBLE);
    }
