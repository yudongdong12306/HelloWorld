...
/**
 * 身高选择
 * Created by dongdong.yu on 2016/8/12.
 */
public class HeightPickerView4Basic<T> extends BasePickerView implements View.OnClickListener,
        SwitchButton.SwitchClickCallBack {
    private static final String TAG = "HeightPickerView4Basic";
    /**
    ...
    public HeightPickerView4Basic(Context context) {
        super(context);
     ...
     */
    public HeightPickerView4Basic(Context context, String title, int unit, int currentValue, boolean isFirst) {
        super(context);
        this.mContext = context;
     ...
     * @param unit    0cm 1IN
     */
    public HeightPickerView4Basic(Context context, String title, int unit) {
        super(context);
        this.mContext = context;
        this.mTitle = title;
        this.unit = unit;
    ...
    public void clickWhichChange(int index) {
        switch (index) {
            case Constant.SWITCH_LEFT:
                ...
                //针对基本信息页面填写身高时,给用户默认值(即用户没有点击确认前).厘米制下默认为1600mm
                if (isFirst) {
                    Log.d(TAG, "clickWhichChange: 进入SWITCH_LEFT,isfirst true");
                    int defaultValue1 = 1600;
                    //cm的默认选中
                    int cmSelect = mCmList.indexOf(defaultValue1);
                    if (cmSelect != -1) {
                        mCmWv.setCurrentItem(cmSelect);
                    } else {
                        Log.d(TAG, "没有选择的:" + defaultValue1);
                    }
//                    //in的默认选中
//                    cmToIn(defaultValue1);
                }
                break;
            case Constant.SWITCH_RIGHT:
                ...
                //针对基本信息页面填写身高时,给用户默认值(即用户没有点击确认前).英寸制下默认为5ft(即60in)。
                if (isFirst) {
                    Log.d(TAG, "clickWhichChange: 进入SWITCH_RIGHT,isfirst true");
                    int defaultValue = 60;
//                inToCm(defaultValue);
                    inToft(defaultValue);
                }
