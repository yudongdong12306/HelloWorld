...
public class WheelView extends View {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textColorOut = getResources().getColor(R.color.picker_black_out);
        textColorCenter = getResources().getColor(R.color.picker_black_center);
...
//        paintCenterText.setTextScaleX(1.1F);
        ...
        paintIndicator.setStrokeWidth((float) 0.5);
    ...
    public final void setTextSizeCenter(float size) {
        if (size > 0.0F) {
            ...
            paintCenterText.setTextSize(textSizeCenter-4);
        }
    }

    public final void setTextSizeOuter(float size) {
        if (size > 0.0F) {
            textSizeOuter = (int) (context.getResources().getDisplayMetrics().density * size);
            paintOuterText.setTextSize(textSizeOuter-8);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            Log.d(TAG, "出错了！adapter.getItemsCount() == 0，联动数据不匹配");
        }
        if (!isLoop) {//不循环的情况
...
//        }
        ...
        while (counter < itemsVisible) {
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                //单位的Label
                if (!TextUtils.isEmpty(label) && !TextUtils.isEmpty(contentText)) {
